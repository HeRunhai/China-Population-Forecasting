...
#
# # 季节性分解
# result = seasonal_decompose(data['Population'], model='', period=1)
# result.plot()
# plt.show()
...
mse = mean_squared_error(y_test, arimax_forecast)
mae = mean_absolute_error(y_test, arimax_forecast)

print(f'Mean Squared Error: {mse}')
print(f'Mean Absolute Error: {mae}')

"""
Mean Squared Error: 14.686773848452646
Mean Absolute Error: 3.070920525282054
"""

print("\n-----------------SARIMA Model with significant features-----------------------------")
# 检查p值，去除p值较大的特征（假设p值>0.05为不显著特征）
significant_features = ['Net migration_lag3', 'Birth Rate_lag1']
print(f"显著特征: {significant_features}")

# 重新训练模型，仅使用显著特征
sign_model = auto_arima(y_train, exogenous=X_train_scaled[significant_features], start_p=1, start_q=1, max_p=5, max_q=5, m=1,
                   start_P=0, seasonal=True, d=2, D=1, trace=True, error_action='ignore',
                   suppress_warnings=True, stepwise=False, n_fits=50, grid_search=True)

print(sign_model.summary())

# Forecast on the test set
arimax_sign_forecast = sign_model.predict(n_periods=test_length, exogenous=X_test_scaled[significant_features])

mse_sign = mean_squared_error(y_test, arimax_sign_forecast)
mae_sign = mean_absolute_error(y_test, arimax_sign_forecast)

print(f'Mean Squared Error: {mse_sign}')
print(f'Mean Absolute Error: {mse_sign}')
...
plt.title('Population Forecast vs Actual (Test Set)')
plt.xlabel('Year')
plt.ylabel('Population')
plt.legend()
plt.show()
...
# 计算新模型的误差
mse_selected = mean_squared_error(y_test, forecast_selected)
mae_selected = mean_absolute_error(y_test, forecast_selected)
