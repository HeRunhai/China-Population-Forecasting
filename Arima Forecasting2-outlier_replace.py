...
plt.legend()
plt.show()
# 图中显示，实际人口数据和移动平均值线非常接近，表明人口增长相对平稳，没有剧烈的波动。这种移动平均方法有助于更好地观察长期趋势，而不被短期的波动所干扰
...
# 4.2 ARIMAX模型
print("\n-----------------SARIMA Model-----------------------------")
# Build ARIMAX model using training data, diff=2, ar(1)
model = auto_arima(y_train, exogenous=X_train_scaled, start_p=1, start_q=1, max_p=5, max_q=5, m=1,
                   start_P=0, seasonal=False, d=2, D=1, trace=True, error_action='ignore',
                   suppress_warnings=True, stepwise=True, n_fits=50, grid_search=True)
...
ljung_box_test = acorr_ljungbox(residuals, lags=[16], return_df=True)

print('\nljung_box_test for residual:\n', ljung_box_test)
